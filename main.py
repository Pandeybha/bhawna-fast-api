...
# --- Service class wrapping all logic ---
class PlayStoreService:
    def __init__(self, dataframe: pd.DataFrame):
        self.df = dataframe

    def row_to_appdata(self, rec: dict) -> AppData:
        return AppData(
    ...
    def list_apps(self, limit: int = 10) -> List[AppData]:
        n = max(1, min(limit, 100))  # clamp 1..100
        subset = self.df.head(n)
        return [self.row_to_appdata(rec) for rec in subset.to_dict(orient="records")]

    def search_apps(self, q: str, limit: int = 20) -> List[AppData]:
        if not APP_COL:
            raise HTTPException(status_code=400, detail="App name column not found in Excel.")
        mask = self.df[APP_COL].astype(str).str.contains(q, case=False, na=False)
        subset = self.df[mask].head(limit)
        if subset.empty:
            raise HTTPException(status_code=404, detail="No matching apps.")
        return [self.row_to_appdata(rec) for rec in subset.to_dict(orient="records")]

    def get_app(self, app_name: str) -> AppData:
        if not APP_COL:
            raise HTTPException(status_code=400, detail="App name column not found in Excel.")
        mask = self.df[APP_COL].astype(str).str.lower() == app_name.lower()
        subset = self.df[mask]
        if subset.empty:
            raise HTTPException(status_code=404, detail="App not found.")
        rec = subset.iloc[0].to_dict()
        return self.row_to_appdata(rec)

# --- Initialize service ---
service = PlayStoreService(df)
